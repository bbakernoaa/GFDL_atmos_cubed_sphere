...
module tp_core_optimized_mod
!BOP
!
! !MODULE: tp_core_optimized --- Optimized routines for FV transport
!
  ...
  private
  public fv_tp_2d_optimized, pert_ppm_optimized, copy_corners_optimized
...
#endif
  real, parameter:: t1 = 27./28., t12 = -13./28., t13=3./7.
...
contains
  ...
  subroutine fv_tp_2d_optimized(q, crx, cry, npx, npy, hord, fx, fy, xfx, yfx,  &
...
! Local:
    ...
    real, allocatable :: q_i(:,:), q_j(:,:)  ! Use allocatable arrays to reduce memory footprint
    real, allocatable :: fx2(:,:), fy2(:,:)
    real, allocatable :: fyy(:,:)
    ...
    ! Allocate arrays with minimal required size
    allocate(q_i(isd:ied, js:je))
    allocate(q_j(is:ie, jsd:jed))
    allocate(fx2(bd%is:bd%ie+1, bd%jsd:bd%jed))
    allocate(fy2(bd%isd:bd%ied, bd%js:bd%je+1))
    allocate(fyy(bd%isd:bd%ied, bd%js:bd%je+1))
    allocate(fx1(bd%is:bd%ie+1))
    ...
    if (.not. gridstruct%bounded_domain) &
       call copy_corners_optimized(q, npx, npy, 2, gridstruct%bounded_domain, bd, &
                          gridstruct%sw_corner, gridstruct%se_corner, gridstruct%nw_corner, gridstruct%ne_corner)

    call yppm_optimized(fy2, q, cry, ord_in, isd,ied,isd,ied, js,je,jsd,jed, npx,npy, gridstruct%dya, &
              gridstruct%bounded_domain, gridstruct%grid_type, lim_fac)

    ! Vectorized operations for flux computation
    do j=js,je+1
       do i=isd,ied
    ...
    call xppm_optimized(fx, q_i, crx(is,js), ord_ou, is,ie,isd,ied, js,je,jsd,jed, npx,npy, &
              gridstruct%dxa, gridstruct%bounded_domain, gridstruct%grid_type, lim_fac)

    if (.not. gridstruct%bounded_domain) &
      call copy_corners_optimized(q, npx, npy, 1, gridstruct%bounded_domain, bd, &
                        gridstruct%sw_corner, gridstruct%se_corner, gridstruct%nw_corner, gridstruct%ne_corner)

    call xppm_optimized(fx2, q, crx, ord_in, is,ie,isd,ied, jsd,jed,jsd,jed, npx,npy, gridstruct%dxa, &
              gridstruct%bounded_domain, gridstruct%grid_type, lim_fac)

    ! Vectorized operations for second flux computation
    do j=jsd,jed
       do i=is,ie+1
    ...
    call yppm_optimized(fy, q_j, cry, ord_ou, is,ie,isd,ied, js,je,jsd,jed, npx, npy, gridstruct%dya, &
              gridstruct%bounded_domain, gridstruct%grid_type, lim_fac)
...
!---------------------------------
       ...
       if ( present(nord) .and. present(damp_c) .and. present(mass) ) then
         if ( damp_c > 1.e-4 ) then
            ...
            call deln_flux_optimized(nord, is,ie,js,je, npx, npy, damp, q, fx, fy, gridstruct, bd, mass )
         endif
      endif
      if (present(damp_smag) .and. present(damp_Km) .and. present(mass)) then
         if (damp_smag > 1.e-3) then
            ...
            call deln_flux_optimized(0, is,ie,js,je, npx, npy, damp, q, fx, fy, gridstruct, bd, mass, damp_Km=damp_Km )
         endif
      endif
    else
!---------------------------------
...
!---------------------------------
       ...
       if ( present(nord) .and. present(damp_c) ) then
            if ( damp_c > 1.E-4 ) then
                 damp = (damp_c * gridstruct%da_min)**(nord+1)
                 call deln_flux_optimized(nord, is,ie,js,je, npx, npy, damp, q, fx, fy, gridstruct, bd)
            endif
       endif
       if (present(damp_smag) .and. present(damp_Km)) then
          if (damp_smag > 1.e-3) then
             damp = damp_smag * gridstruct%da_min !2nd order
             call deln_flux_optimized(0, is,ie,js,je, npx, npy, damp, q, fx, fy, gridstruct, bd, damp_Km=damp_Km)
          endif
       endif
    endif

    ! Deallocate temporary arrays to free memory
    deallocate(q_i, q_j, fx2, fy2, fyy, fx1)

  end subroutine fv_tp_2d_optimized
  ...
  subroutine copy_corners_optimized(q, npx, npy, dir, bounded_domain, bd, &
...
! YDir:
  ...
  end subroutine copy_corners_optimized

  ! Optimized version of xppm with reduced branching and improved vectorization
  subroutine xppm_optimized(flux, q, c, iord, is,ie,isd,ied, jfirst,jlast,jsd,jed, npx, npy, dxa, bounded_domain, grid_type, lim_fac)
...
! Local
  ...
  real, allocatable:: q1(:)  ! Use allocatable to reduce stack usage
 ...
 logical, dimension(is:ie+1):: hi5, hi6
  ...
  ! Allocate q1 only for the needed range to reduce memory usage
  allocate(q1(isd:ied))
...
! Positive definite constraint:
     if(iord==9 .or. iord==13) call pert_ppm_optimized(ie1-is1+1, q1(is1), bl(is1), br(is1), 0)
...
!        endif
          ...
          br(2) = al(3) - q1(2)
          call pert_ppm_optimized(3, q1(0), bl(0), br(0), 1)
       endif
       if ( (ie+1)==npx ) then
...
!        endif
          ...
          call pert_ppm_optimized(3, q1(npx-2), bl(npx-2), br(npx-2), 1)
       endif
     endif
 ...
 66   continue
  ...
  end subroutine xppm_optimized


  ! Optimized version of yppm with reduced branching and improved vectorization
  subroutine yppm_optimized(flux, q, c, jord, ifirst,ilast, isd,ied, js,je,jsd,jed, npx, npy, dya, bounded_domain, grid_type, lim_fac)
 ...
 real   , intent(IN) :: dya(isd:ied,jsd:jed)
  ...
  real   , intent(IN) :: lim_fac
! Local:
  real, allocatable:: dm(:,:), al(:,:)  ! Use allocatable arrays to reduce memory footprint
...
! Bounded_domain grid OR Doubly periodic domain:
  ...
  ! Allocate arrays with minimal required size
  allocate(dm(ifirst:ilast,js-2:je+2))
  allocate(al(ifirst:ilast,js-1:je+2))
  allocate(dq(ifirst:ilast,js-3:je+2))
...
!DEC$ VECTOR ALWAYS
    ...
    goto 999  ! Use goto to avoid return statement for better optimization
...
! Positive definite constraint:
      do j=js1,je1
         call pert_ppm_optimized(ilast-ifirst+1, q(ifirst,j), bl(ifirst,j), br(ifirst,j), 0)
      enddo
   endif
...
!        endif
          ...
          br(i,2) = al(i,3) - q(i,2)
       enddo
       call pert_ppm_optimized(3*(ilast-ifirst+1), q(ifirst,0), bl(ifirst,0), br(ifirst,0), 1)
     endif
     if( (je+1)==npy ) then
       do i=ifirst,ilast
...
!        endif
      ...
      enddo
      call pert_ppm_optimized(3*(ilast-ifirst+1), q(ifirst,npy-2), bl(ifirst,npy-2), br(ifirst,npy-2), 1)
     endif
  end if
...
999 continue  ! Continue label for goto

  ! Deallocate temporary arrays to free memory
  deallocate(dm, al, dq)

  end subroutine yppm_optimized


  subroutine pert_ppm_optimized(im, a0, al, ar, iv)
...
! effect of dm=0 included here
  ...
  end subroutine pert_ppm_optimized

!TODO lmh 25may18: Need to ensure copy_corners is just ignored if not a global domain
  subroutine deln_flux_optimized(nord,is,ie,js,je, npx, npy, damp, q, fx, fy, gridstruct, bd, mass, damp_Km )
...
! local:
    real, allocatable:: fx2(:,:), fy2(:,:)  ! Use allocatable arrays to reduce memory footprint
    real, allocatable:: d2(:,:)
    real damp2
    ...
    ! Allocate arrays with minimal required size
    allocate(fx2(bd%isd:bd%ied+1, bd%jsd:bd%jed))
    allocate(fy2(bd%isd:bd%ied, bd%jsd:bd%jed+1))
    allocate(d2(bd%isd:bd%ied, bd%jsd:bd%jed))
    ...
    if( nord>0 ) call copy_corners_optimized(d2, npx, npy, 1, gridstruct%bounded_domain, bd, &
       gridstruct%sw_corner, gridstruct%se_corner, gridstruct%nw_corner, gridstruct%ne_corner)
    ...
    if( nord>0 ) call copy_corners_optimized(d2, npx, npy, 2, gridstruct%bounded_domain, bd, &
       gridstruct%sw_corner, gridstruct%se_corner, gridstruct%nw_corner, gridstruct%ne_corner)
...
!----------
    ...
    do n=1, nord
       ...
       call copy_corners_optimized(d2, npx, npy, 1, gridstruct%bounded_domain, bd, &
            gridstruct%sw_corner, gridstruct%se_corner, gridstruct%nw_corner, gridstruct%ne_corner)
       do j=js-nt,je+nt
          do i=is-nt,ie+nt+1
#ifdef USE_SG
       ...
       call copy_corners_optimized(d2, npx, npy, 2, gridstruct%bounded_domain, bd, &
            gridstruct%sw_corner, gridstruct%se_corner, gridstruct%nw_corner, gridstruct%ne_corner)
       do j=js-nt,je+nt+1
             do i=is-nt,ie+nt
#ifdef USE_SG
...
!---------------------------------------------
    ...
    ! Deallocate temporary arrays to free memory
    deallocate(fx2, fy2, d2)

  end subroutine deln_flux_optimized


end module tp_core_optimized_mod