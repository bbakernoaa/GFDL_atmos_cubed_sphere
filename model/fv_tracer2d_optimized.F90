...
module fv_tracer2d_optimized_mod
    use tp_core_optimized_mod, only: fv_tp_2d_optimized, copy_corners_optimized
...
public :: tracer_2d_optimized, tracer_2d_nested_optimized, tracer_2d_1L_optimized

contains

!-----------------------------------------------------------------------
! !ROUTINE: Perform 2D horizontal-to-lagrangian transport - Optimized version
!-----------------------------------------------------------------------

subroutine tracer_2d_1L_optimized(q, dp1, mfx, mfy, cx, cy, gridstruct, bd, domain, npx, npy, npz,   &
...
! Local Arrays
       real, allocatable :: qn2(:,:,:)   ! 3D tracers - use allocatable to reduce memory footprint
       real, allocatable :: dp2(:,:)
       real, allocatable :: fx(:,:)
       real, allocatable :: fy(:,:)
       real, allocatable :: ra_x(:,:)
       real, allocatable :: ra_y(:,:)
       real, allocatable :: xfx(:,:,:)
       real, allocatable :: yfx(:,:,:)
       ...
       ! Allocate arrays with minimal required size to reduce memory footprint
       allocate(qn2(bd%isd:bd%ied, bd%jsd:bd%jed, nq))
       allocate(dp2(bd%is:bd%ie, bd%js:bd%je))
       allocate(fx(bd%is:bd%ie+1, bd%js:bd%je))
       allocate(fy(bd%is:bd%ie, bd%js:bd%je+1))
       allocate(ra_x(bd%is:bd%ie, bd%jsd:bd%jed))
       allocate(ra_y(bd%isd:bd%ied, bd%js:bd%je))
       allocate(xfx(bd%is:bd%ie+1, bd%jsd:bd%jed, npz))
       allocate(yfx(bd%isd:bd%ied, bd%js:bd%je+1, npz))
       allocate(cmax(npz))
       allocate(frac(npz))
...
!$OMP                          private(fx,fy)
         ...
         if ( nsplt /= 1 ) then
            ...
            call fv_tp_2d_optimized(qn2(isd,jsd,iq), cx(is,jsd,k), cy(isd,js,k), &
            ...
            call fv_tp_2d_optimized(q(isd,jsd,k,iq), cx(is,jsd,k), cy(isd,js,k), &
                          npx, npy, hord, fx, fy, xfx(is,jsd,k), yfx(isd,js,k), &
                          gridstruct, bd, ra_x, ra_y, lim_fac, mfx=mfx(is,js,k), mfy=mfy(is,js,k))
            do j=js,je
               do i=is,ie
...
!$OMP                           private(rdt)
    ...
    deallocate(qn2, dp2, fx, fy, ra_x, ra_y, xfx, yfx, cmax, frac)

end subroutine tracer_2d_1L_optimized


subroutine tracer_2d_optimized(q, dp1, mfx, mfy, cx, cy, gridstruct, bd, domain, npx, npy, npz,   &
                      nq,  hord, q_split, dt, id_divg_mean, q_pack, dp1_pack, nord_tr, trdm, lim_fac)
...
!$OMP                           private(rdt)
    ...
    ! Deallocate temporary arrays to free memory
    deallocate(dp2, fx, fy, ra_x, ra_y, xfx, yfx, cmax, frac, ksplt)

end subroutine tracer_2d_optimized


subroutine tracer_2d_nested_optimized(q, dp1, mfx, mfy, cx, cy, gridstruct, bd, domain, npx, npy, npz,   &
...
!$OMP parallel do default(none) shared(is,ie,js,je,npz,dp1,xfx,yfx,rarea,rdt)
    ...
    ! Deallocate temporary arrays to free memory
    deallocate(dp2, fx, fy, ra_x, ra_y, xfx, yfx, cmax)

  end subroutine tracer_2d_nested_optimized

end module fv_tracer2d_optimized_mod